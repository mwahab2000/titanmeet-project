...
export const PublicDressCodeSection: React.FC<Props> = ({ data, className = "" }) => {
  ...
  return (
    <MotionReveal id="dress-code" className={`max-w-5xl mx-auto px-6 sm:px-8 py-24 ${className}`}>
      <div className="flex items-center gap-4 mb-12">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
            <Shirt className="h-5 w-5 text-primary" />
          </div>
          <h2 className="text-3xl md:text-4xl font-bold font-display tracking-tight">Dress Code</h2>
        </div>
        <div className="flex-1 h-px bg-border" />
      </div>

      <div className={`grid gap-5 ${multiDay ? "md:grid-cols-2" : ""}`}>
        {dressCode.map((dc, i) => (
          <MotionRevealItem key={i} index={i} className="rounded-2xl border border-border bg-card p-6 sm:p-7 space-y-4 hover:shadow-[var(--shadow-elevated)] transition-all duration-300">
            {multiDay && (
            ...
            <h3 className="text-lg font-bold font-display">{DRESS_TYPE_LABELS[dc.dressType] ?? dc.dressType}</h3>
              ...
              <div className="grid grid-cols-3 gap-2 pt-2">
                {dc.referenceImages.map((img, j) => (
                  <div key={j} className="relative group cursor-pointer rounded-xl overflow-hidden" onClick={() => openLightbox(dc.referenceImages, j)}>
                    <img src={img} alt={`Dress code ${j + 1}`} className="w-full h-24 object-cover transition-transform duration-300 group-hover:scale-105" onError={(e) => { (e.target as HTMLImageElement).src = fallback; }} />
