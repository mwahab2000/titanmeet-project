...
export const PublicAgendaSection: React.FC<Props> = ({ data, className = "" }) => {
  ...
  return (
    <MotionReveal id="agenda" className={`max-w-5xl mx-auto px-4 sm:px-8 py-16 sm:py-24 ${className}`}>
      <div className="flex items-center gap-3 sm:gap-4 mb-8 sm:mb-12">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold font-display tracking-tight">Agenda</h2>
        <div className="flex-1 h-px bg-border" />
      </div>

      {days.map((day) => (
        <div key={day} className="mb-8 sm:mb-12 last:mb-0">
          {days.length > 1 && (
          ...
          <div className="space-y-3">
            {grouped[day].map((item, i) => (
              <MotionRevealItem
                key={item.id}
                index={i}
                className="group relative flex flex-col sm:flex-row gap-3 sm:gap-5 p-4 sm:p-6 rounded-2xl border border-border bg-card hover:shadow-[var(--shadow-elevated)] hover:border-primary/15 transition-all duration-300"
              >
                {/* Time chip */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.description && <p className="text-sm text-muted-foreground mt-1.5 sm:mt-2 leading-relaxed line-clamp-3 sm:line-clamp-2">{item.description}</p>}
                  {item.speakerName && (
                    <p className="text-xs text-primary/80 mt-2 sm:mt-2.5 font-medium flex items-center gap-1.5">
